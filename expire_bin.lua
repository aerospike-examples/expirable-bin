...
-- (*) create: set to true to create new exp_bins
--
-- Return:
-- 1 = error
-- 0 = success
-- =========================================================================
function put(rec, bin, val, bin_ttl, create)
	local meth = "put";
	GP=F and debug("[BEGIN]<%s> bin:%s value:%s ttl:%s exp_create:%s", meth, bin, tostring(val), tostring(bin_ttl), tostring(create));
	local exp_create;
	if (create == 0) then
		exp_create = true;
	else
